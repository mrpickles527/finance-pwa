          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", backdropFilter: "blur(10px)", zIndex: 200, display: "flex", flexDirection: "column", justifyContent: "flex-end", maxWidth: 430, left: "50%", transform: "translateX(-50%)", width: "100%" }} onClick={() => setEditAccount(null)}>
            <div style={{ background: "#111", borderRadius: "24px 24px 0 0", padding: "28px 20px 48px", border: `1px solid ${C.border}`, width: "100%" }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <button onClick={() => setEditAccount(p => { const n = parseFloat(p.saldo) || 0; return { ...p, saldo: String(-n) }; })} style={{ width: 48, height: 48, borderRadius: 12, border: `1px solid ${(parseFloat(editAccount.saldo) || 0) < 0 ? C.red : C.border}`, background: (parseFloat(editAccount.saldo) || 0) < 0 ? `${C.red}22` : "#141414", color: (parseFloat(editAccount.saldo) || 0) < 0 ? C.red : C.textDim, fontSize: 22, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>±</button>
                  <input type="number" inputMode="decimal" value={editAccount.saldo} onChange={e => setEditAccount(p => ({ ...p, saldo: e.target.value }))} style={{ background: "#141414", border: `1px solid ${C.border}`, borderRadius: 12, padding: "13px 14px", color: (parseFloat(editAccount.saldo) || 0) < 0 ? C.red : C.text, fontFamily: "'Space Mono',monospace", fontSize: 22, flex: 1, outline: "none", textAlign: "center" }} />
                </div>
